...
class Mysql extends \PDO
{
    ...
    public $throwExp = false; // 默认不抛异常
    public $isWriteLog = true; // 写日志推微信,可根据业务场景重写为匿名函数
     ...
     * @param array $idata 数据键值对数组，如['name'=>'','age'=>18]其中键为表字段，值为数值。支持批量插入 [ ['name'=>'xiaoxiao','age'=>8], ['name'=>'','age'=>18]]
     ...
     * 删除数据
     * @param string $table
     * @param array $idata 条件键值对数组，如['name'=>'','age'=>18]其中键为表字段，值为数值，条件之间的关系为and
     * @param bool $fetch 是否只记录SQL语句而不执行
     ...
     * @param array $where 条件键值对数组，如['name'=>'','age'=>18]其中键为表字段，值为数值，条件之间的关系为and
    ...
    {
        // 记录sql往前提，避免重连的时候直接return没有记录到sql
        if (APP_DEBUG || !empty($this->errorInfo['err_no'])) {
            trace($this->lastSql . ($data ? '  ,data :: ' . json_encode($data, JSON_UNESCAPED_UNICODE) : ''), 'sql');
        }
    ...
    private function log($data = [])
    {
        ...
        if ((($this->isWriteLog instanceof \Closure) && (($this->isWriteLog)($extend)))
            || $this->isWriteLog === true
        ) {
            Exception::report($e, $extend);
        }
